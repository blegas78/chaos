###########################################################
#                  Find OpenCV Library See http://sourceforge.net/projects/opencvlibrary/ 
# ----------------------------------------------------------
#
## 1: Setup:
...
#set(OpenCV_DIR /opt/local/include/)
...
if(EXISTS "${OpenCV_DIR}")
	...
	#	if(APPLE)
	                set(OPENCV_LIB_COMPONENTS core imgproc highgui ml video features2d calib3d objdetect contrib legacy flann )
	#		message("--	Configuring OpenCV for Apple")
	#	else(APPLE)
	#		set(OPENCV_LIB_COMPONENTS cxcore cv ml highgui cvaux)
	#		message("--	Configuring OpenCV for Ubuntu(Linux)")
	#	endif(APPLE)

                find_path(	OpenCV_INCLUDE_DIR cv.h 
				HINTS /usr/local/ /usr/
				PATHS "${OpenCV_DIR}" 
				PATH_SUFFIXES "include" "include/opencv" DOC "")

#		message("-- Possible OpenCV include: ${OpenCV_INCLUDE_DIR}") 
		...
		if(APPLE)
#			message ("-- WARNING:  Cannot get OpenCV version in apple...")
		else(APPLE)
...
#                	set(CVLIB_SUFFIX "${OpenCV_VERSION_MAJOR}${OpenCV_VERSION_MINOR}${OpenCV_VERSION_PATCH}")
			message ("-- WARNING: Having trouble detecting OpenCV version in Ubuntu...")
		endif(APPLE)
...
#                find_library(	OpenCV_${__CVLIB}_LIBRARY_DEBUG NAMES "${__CVLIB}${CVLIB_SUFFIX}d" "lib${__CVLIB}${CVLIB_SUFFIX}d"
                find_library(	OpenCV_${__CVLIB}_LIBRARY_DEBUG NAMES "${__CVLIB}d" "lib${__CVLIB}d" "opencv_${__CVLIB}d" "libopencv_${__CVLIB}d"
				HINTS /usr/lib/ /usr/local/lib/ /opt/local/lib/
				PATHS "${OpenCV_DIR}/lib" NO_DEFAULT_PATH)
 #               find_library(	OpenCV_${__CVLIB}_LIBRARY_RELEASE NAMES "${__CVLIB}" "lib${__CVLIB}" 
                find_library(	OpenCV_${__CVLIB}_LIBRARY_RELEASE NAMES "${__CVLIB}" "lib${__CVLIB}" "opencv_${__CVLIB}" "libopencv_${__CVLIB}"
				HINTS /usr/lib/ /usr/local/lib/ /opt/local/lib/
				PATHS "${OpenCV_DIR}/lib" NO_DEFAULT_PATH)
			...
			#message("-- FATAL ERROR: OpenCV library lib${__CVLIB}${CVLIB_SUFFIX} not found!")
                endif()
